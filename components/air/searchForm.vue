...
export default {
  ...
  methods: {
    ...
    // 出发城市读入的时候会触发
    // value:输入框的值
    // cb:回调函数,必须是一个对象，对象中必须要有value属性
    queryDepartSearch(value, cb) {
      ...
      this.$axios({
        ...
        const newData = data.map(v => {
          v.value = v.name.replace("市", ""); // 乌鲁木齐市
    ...
    // 目标城市输入框获得焦点触发 : value 是选中的值，cb是回调函数，接收要展示的列表
    ...
    handleSubmit() {
      ...
      const rules = {
        ...
        // 循环rules这个对象，并判断对象属性的value如果是空的，打印出message错误信息
        let valid = true;
